...
void SkinChanger::SetWeaponSkin(int weaponID, DWORD64 weapon,
                                DWORD64 weaponGameSceneNode,
                                int MeshGroupMask) {
  // Set Mask
  if (MeshGroupMask != 2) {
    int Mask = 2;
    ProcessMgr.WriteMemory<int>(weaponGameSceneNode + 0x160 +
                                    Offset::WeaponBaseData.m_MeshGroupMask,
                                Mask);
  }
  ...
  ProcessMgr.WriteMemory(weapon + Offset::EconEntity.AttributeManager +
...
void SkinChanger::Run(const CEntity &LocalPlayer, CGame Game) {
  ...
  for (int i = 0; i < WeaponSize; i++) {
    ...
    int Mask = 2;
    ProcessMgr.WriteMemory(ViewGameSenceNode + 0x160 +
                               Offset::WeaponBaseData.m_MeshGroupMask,
                           Mask);
  }
}